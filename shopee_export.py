...
def export_rendered_video_to_shopee_files(proj_dir, out_file, config=None, default_status="Chưa đăng"):
    ...
    cursor.execute('''
        INSERT INTO shopee_jobs (project_id, video_name, product_name, links, caption, status)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (db_proj_id, video_name, product_info["product_name"], final_link_cell, product_info["caption"], default_status))
...
def delete_shopee_jobs(video_names, csv_path=None, config=None):
    ...
    cursor.execute(f"DELETE FROM shopee_jobs WHERE video_name IN ({placeholders})", tuple(video_names))
