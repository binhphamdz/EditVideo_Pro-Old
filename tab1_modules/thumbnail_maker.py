...
class ThumbnailHandler:
    ...
    def generate(self, project_id, broll_dir, trash_dir, act_files, tr_files, render_request_id=None, rebuild_when_done=True):
        ...
        def process_folder(files_list, target_dir, data_dict, updated_dict):
            nonlocal changed
            thumb_dir = os.path.join(target_dir, ".thumbnails")
            os.makedirs(thumb_dir, exist_ok=True)
            
            for vid_name in files_list:
                progress_key = (project_id, target_dir, vid_name)
                with self._progress_lock:
                    if progress_key in self._in_progress:
                        continue
                ...
                try:
                    if vid_name not in data_dict or not isinstance(data_dict[vid_name], dict):
                        ...
                        changed = True
                        updated_dict[vid_name] = dict(data_dict[vid_name])
        ...
        try:
            ...
            process_folder(act_files, broll_dir, p_data["videos"], updated_videos)
            process_folder(tr_files, trash_dir, p_data["trash"], updated_trash)
